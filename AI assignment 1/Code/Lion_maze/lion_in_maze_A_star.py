...
# Node class to represent the state, parent, path cost and heuristic cost
class node:
	def __init__(self,current_position,parent,cost,n):
		self.state = current_position
		self.parent = parent
		self.path_cost = cost

		row = current_position/10
		col = current_position%10

		# Manhattan heuristic is followed
		self.heuristic_cost = abs(n - row) + abs(n - col)
...
# Comparator function to maintain frontier as a priority queue w.r.t evaluation function
...
def search_algo(n, maze, start, end):
	...
	frontier_queue = []
	root_node = node(start,-1,0,n)
	# frontier_queue.append(root_node)	#initialize frontier
	...
	while len(frontier_queue) != 0:
		...
		# Also, frontier maintains a priority queue w.r.t evaluation function
		...
		if row == 0:
			if col == 0:
				if maze[row][col+1] == 0: 	#check only for valid neighbors
					if explored_set[row][col+1] == False and check_frontier(pos+1,frontier_queue): 	#check for unvisited node
						frontier_queue.append(node(pos+1,current_node,current_node.path_cost + 2,n))
						frontier_queue.sort(key = cost_comparator)

				if maze[row+1][col] == 0: 	
					if explored_set[row+1][col] == False and check_frontier(pos+10,frontier_queue): 	
						frontier_queue.append(node(pos+10,current_node,current_node.path_cost + 3,n))
						frontier_queue.sort(key = cost_comparator)
			...
			if col == 0:
				...
				if maze[row-1][col] == 0: 	
					if explored_set[row-1][col] == False and check_frontier(pos-10,frontier_queue): 	
						frontier_queue.append(node(pos-10,current_node,current_node.path_cost + 2,n))
						frontier_queue.sort(key = cost_comparator)
