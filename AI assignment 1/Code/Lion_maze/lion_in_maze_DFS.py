...
# Node class to represent the state and the parent
class node:
...
def search_algo(n, maze, start, end):
	...
	while len(frontier_queue) != 0:
		...
		# In DFS, we pop the front of the frontier, essentially it is a stack
		current_node = frontier_queue.pop(-1)
			...
			if col == 0:
				if maze[row][col+1] == 0: 	#check only for valid neighbors
					if explored_set[row][col+1] == False and check_frontier(pos+1,frontier_queue): 	#check for unvisited node
						frontier_queue.append(node(pos+1,current_node))

				if maze[row+1][col] == 0: 	
					if explored_set[row+1][col] == False and check_frontier(pos+10,frontier_queue): 	
						frontier_queue.append(node(pos+10,current_node))
...
# =============================================================================
	...
	if current_node.state == end:
		...
		total_cost = 3 * (len(path)-1) #step cost in DFS is 3

		# Various runtime statistics 
		print('Total path cost is:' + str(total_cost))
